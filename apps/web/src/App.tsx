...
const skills = [
  ...
  { icon: '⛏', name: '自动挖矿与资源采集', key: 'mining', description: '在指定长方体区域内按白名单或黑名单采集，容器与流体默认保护并在必要时安全封堵。', commands: ['area set <两组坐标>', 'area mode blacklist|whitelist', 'area start/stop', 'unseal'], status: '已启用' },
...
const skillGuides: Record<string, { examples: string[]; notes: string[]; next: string[] }> = {
  ...
  mining: { examples: ['Shinano area set -30 60 -30 30 90 30', 'Shinano area mode blacklist', 'Shinano area deny diamond_ore chest', 'Shinano area start', 'Shinano unseal'], notes: ['默认黑名单会保护箱子、容器、床、基岩、传送门和水/岩浆；白名单模式必须明确允许方块。', '遇到水或岩浆时会先用背包中的圆石等方块封堵；本轮结束后封堵物仍保留，确认安全后再执行 unseal。', '区域体积有上限，路径、未加载区块、缺少工具或缺少封堵方块时会暂停而不是冒险继续。'], next: ['矿区分层路线规划', '耐久度与经验阈值'] },
