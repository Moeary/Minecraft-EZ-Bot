  ...
  return (
    <main className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      <aside className="sidebar">
        ...
        <div className="sidebar-bots"><div className="sidebar-bots-title"><span>运行中的机器人</span><button onClick={() => setView('bots')}>全部</button></div>{bots.slice(0, 6).map((bot) => <button className="mini-bot" key={bot.id} onClick={() => selectBot(bot.id)}><span className={`state-dot state-${bot.state}`} /><span><strong>{bot.displayName}</strong><small>{bot.state === 'online' ? (bot.killAura ? '战斗中' : bot.fishing ? '钓鱼中' : '待命') : stateLabel(bot.state)}</small></span><b>›</b></button>)}{bots.length > 6 && <button className="sidebar-more" onClick={() => setView('bots')}>＋ {bots.length - 6} 个机器人 →</button>}</div>
...
function BotEditor({ editorMode, setEditorMode, form, updateForm, submitBot }: { editorMode: 'add' | 'edit'; setEditorMode: (mode: 'add' | 'edit' | null) => void; form: BotForm; updateForm: <K extends keyof BotForm>(key: K, value: BotForm[K]) => void; submitBot: (event: FormEvent) => Promise<void> }) {
  return <div className="editor-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setEditorMode(null); }}>
    <form className="editor-modal" role="dialog" aria-modal="true" aria-labelledby="bot-editor-title" onSubmit={submitBot}>
      ...
      <div className="editor-section"><h3>登录与认证</h3><div className="form-grid"><label className="wide">账号 / 用户名<input value={form.username} onChange={(event) => updateForm('username', event.target.value)} required placeholder="Microsoft 邮箱或离线用户名" /><small className="field-hint">Microsoft 登录缓存会按 bot ID 独立保存在 data/auth/ 下。</small></label><label>认证方式<select value={form.auth} onChange={(event) => updateForm('auth', event.target.value)}><option value="microsoft">Microsoft OAuth</option><option value="offline">离线 / Cracked</option><option value="mojang">Mojang（兼容）</option></select></label><label>第三方验证<select defaultValue="auto"><option value="auto">自动处理（推荐）</option><option value="manual">手动确认</option><option value="none">不启用</option></select></label></div><div className="security-note">▣ 认证状态由 Mineflayer authflow 管理。首次登录请在终端完成设备码验证，不要把 token 提交到 Git。</div></div>
...
function DetailPage({ bots, onSelectBot, selected, selectedDefinition, logs, command, setCommand, submitCommand, run, previewMode, setPreviewMode, viewerUrl, openEdit }: { bots: BotStatus[]; onSelectBot: (id: string) => void; selected: BotStatus; selectedDefinition?: BotDefinition; logs: LogEntry[]; command: string; setCommand: (value: string) => void; submitCommand: (event: FormEvent) => Promise<void>; run: (action: () => Promise<unknown>, refresh?: boolean) => Promise<void>; previewMode: 'status' | 'viewer'; setPreviewMode: (mode: 'status' | 'viewer') => void; viewerUrl: string; openEdit: () => void }) {
  ...
  const task = selected.killAura ? '正在战斗' : selected.fishing ? '正在钓鱼' : '待命中';
    ...
    <div className="detail-layout detail-layout-full">
      <div className="detail-main">
        <div className="card preview-card">
          ...
          {previewMode === 'viewer' && viewerUrl ? <iframe key={`${selected.id}-${viewerRevision}`} className="viewer-frame" title={`${selected.displayName} viewer`} src={viewerUrl} /> : <div className="status-preview"><div className="scene-grid"><div className="scene-sun">☀</div><svg className="bot-illustration" viewBox="0 0 180 220" role="img" aria-label={`${selected.displayName} 状态插画`}><defs><linearGradient id="botShirt" x1="0" x2="1"><stop offset="0" stopColor="#356fa0" /><stop offset="1" stopColor="#235477" /></linearGradient><linearGradient id="botSkin" x1="0" x2="1"><stop offset="0" stopColor="#e7ab78" /><stop offset="1" stopColor="#bd7650" /></linearGradient></defs><ellipse cx="90" cy="206" rx="51" ry="8" fill="rgba(27,76,45,.18)" /><rect x="55" y="31" width="70" height="62" rx="5" fill="url(#botSkin)" stroke="#633d2b" strokeWidth="6" /><path d="M57 43V30h66v16c-9-7-19-10-30-7-12 3-22 3-36 4Z" fill="#4b332b" /><rect x="70" y="57" width="12" height="8" rx="2" fill="#172b31" /><rect x="98" y="57" width="12" height="8" rx="2" fill="#172b31" /><path d="M81 78c6 4 12 4 18 0" fill="none" stroke="#77412d" strokeWidth="3" strokeLinecap="round" /><path d="M43 98h94l13 66H30l13-66Z" fill="url(#botShirt)" stroke="#20445e" strokeWidth="6" /><path d="M43 102 20 126l17 15 21-25M137 102l23 24-17 15-21-25" fill="#2d638e" stroke="#20445e" strokeWidth="6" strokeLinejoin="round" /><path d="M59 164h28v36H51l8-36ZM93 164h28l8 36H93v-36Z" fill="#604438" stroke="#3d302b" strokeWidth="6" /><rect x="66" y="110" width="48" height="43" rx="4" fill="rgba(133,214,150,.18)" /></svg><div className="scene-label"><strong>{task}</strong><span className="scene-dimension">{dimensionLabel(selected.dimension)}</span><span>{coordinate}</span></div></div></div>}
